...
function render() {
  ...
  document.getElementById("app").innerHTML = `
    ...
    ${
      ...
      <div class="wt-rate-card"><p class="wt-rl">${t("rateEur")}</p><p class="wt-rv">${fmtNum(rates.eurPerUsd, 2)}</p></div>
      <div class="wt-rate-card"><p class="wt-rl">${t("rateSar")}</p><p class="wt-rv">${fmtNum(rates.sarPerUsd, 2)}</p></div>
      ...
      <table class="wt-table">
        ...
        ${(groupFilter ? orderedAssets.filter((a) => a.group === groupFilter) : orderedAssets)
          .map((a, idx, visible) => {
            ...
            ${isColHidden("apy") ? "" : assetGeneratesReturn(a.id) ? `<td class="wt-apy-cell" title="${t("apyHint")}"><button type="button" class="wt-apy-set-link" onclick="openReturnPanel('${a.id}')">${(() => { const displayRate = currentTierRate(returnConfig[a.id] || {}, parseDateStr(todayLocalStr())); return displayRate != null ? fmtNum(displayRate, 2) + "%" : apy[a.id] ? fmtNum(apy[a.id], 2) + "%" : t("setApyLink"); })()}</button>${apy[a.id] && returnConfig[a.id] && returnConfig[a.id].rateBasis ? `<div class="wt-proj-date" dir="auto" title="${t("rateBasisHint")}">${t("rateBasisShort")[returnConfig[a.id].rateBasis]}</div>` : ""}</td>` : `<td class="wt-apy-cell">—</td>`}
            ${isColHidden("unitPrice") ? "" : `<td class="wt-price-cell">${fmtNum(p, 2)}</td>`}
